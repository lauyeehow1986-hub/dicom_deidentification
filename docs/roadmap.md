...
| **3** | Pixel / burned-in PHI | Frame/RGB/multiframe decode (incl. JPEG2000/RLE); Pixels viewer with OCR-proposed + manual redaction boxes; decompress→redact→write valid uncompressed DICOM; audio/waveform strip. **(done)** — OCR degrades gracefully w/o Tesseract; optional defacing (MRI, opt-in, FOV-gated) **done** — CT defacing is future scope. |
| **3.5** | NIfTI hardening | Beyond the header free-text scrub: strip **all** header extensions (can embed a whole DICOM dataset / freeform PHI); preserve the input's **NIfTI-1/2 class** (no silent downcast); de-identify (not drop) paired **dcm2niix/BIDS `.json` sidecars** — identifier keys removed via the DICOM action map, date keys shifted/removed per profile, remaining string values scrubbed by the text scanner seeded on the sidecar's own IDs; **sanitise identifier tokens in output filenames** (name/NRIC/phone tokens → stable pseudonym, volume + sidecar renamed in lockstep). Corpus + survivor scan extended to plant/verify each surface (extension, NIfTI-2 descrip, sidecar, PHI filename). Burned-in-pixel OCR on NIfTI arrays done (scans the shortest/through-plane axis; all-three-plane scanning is future scope). **(done)** |
